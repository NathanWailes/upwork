...
def get_classes_seat(room,clas): #this function get LIST of students from specified CLASS/COLLEGE
    post=requests.post(url="https://centres.muhs.edu.in/mnk/ug_res/cres_x4.asp",data={'cr':'BHMS-2015','cl':clas,'sn':room})
...
def get_scores(seats,clas): #this function get result LIST of seats from get_classes_seat and crawl info for every seat/student
    students={}
    for seat in seats:
        post_s=requests.post(url="https://centres.muhs.edu.in/mnk/ug_res/res_x4.asp",data={'cr':'BHMS-2015','cl':clas,'sn':seat})
        ...
        for subj in [i.text for i in soup_seat.html.body.findAll("td",attrs={'align':"left",'colspan':'2'})][1:-2]:
            student[subj+"_theor"]=soup_seat.html.body.table.nextSibling.nextSibling.findAll("td",text=subj)[0].nextSibling.nextSibling.text.strip("\n")
            student[subj+"_pract"]=soup_seat.html.body.table.nextSibling.nextSibling.findAll("td",text=subj)[0].nextSibling.nextSibling.nextSibling.nextSibling.text.strip("\n")
            student[subj+"_total"]=soup_seat.html.body.table.nextSibling.nextSibling.findAll("td",text=subj)[0].nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.text.strip("\n")
            student['grand_total']=soup_seat.html.body.table.nextSibling.nextSibling.findAll("td",text='GRAND TOTAL')[0].nextSibling.nextSibling.text.replace("\n","").replace("\r","").replace("\xa0","")
            student['stud_result']=soup_seat.html.body.table.nextSibling.nextSibling.findAll("td",text='RESULT')[0].nextSibling.nextSibling.text.replace("\n","").replace("\r","").replace("\xa0","")
